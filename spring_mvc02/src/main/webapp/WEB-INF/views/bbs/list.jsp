...
<body>
				...
				<c:choose>
					...
					<c:otherwise>
						<c:forEach var="k" items="${list}" varStatus="s">
							<tr>
								<td>${paging.totalRecords - ((paging.nowPage-1)*paging.numPerPage + s.index)}</td>
								
								
								<c:choose>
									<c:when test="${k.active == 1}">
										<td style="color:gray"> 삭제된 게시글 입니다.</td>
									</c:when>
									<c:otherwise>
										<td><a href="/detail?b_idx=${k.b_idx}&cPage=${paging.nowPage}">${k.subject}</a></td>																		
									</c:otherwise>
								</c:choose>
								<c:choose>
									<c:when test="${k.writer == null || k.writer == '' }">
										<td>익명</td>
									</c:when>
									<c:otherwise>
										<td>${k.writer}</td>					
	...
	<script>
		$("#pageNum").change(function() {
		    let selectedPageNum = $(this).val();
		    location.href = `/bbs?p_num=${selectedPageNum}`;
		});
	</script>
</body>
</html>
