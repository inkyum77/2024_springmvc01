			...
			<tbody>
				...
				<tr>
					<td colspan="2">
						...
						<input type="reset" value="삭제" onclick="bbs_delete(this.form)">
	...
	<div style="padding:10px; width: 500px; margin: 0 auto;">
		<form action="/comment_insert" method="post">
			<fieldset>
				...
				<p>내용 : <textarea rows="3" cols="40" type="text" name="content"></textarea></p>
				<!-- 댓글 저장시 어떤 글이 원글인지 저장해야함. -->
				<input type="hidden" name="b_idx" value="${bvo.b_idx}">
				<input type="hidden" name="cPage" value="${cPage}">
	...
	<c:forEach items="${clist}" var="c">
		<div style="border: 1px solid #cc00cc; width:580px; margin: 5px; padding:5px;">
			<form action="/comment_delete" method="post">
				<p>이름 : ${c.writer}</p>
				<p>내용 : <pre>${c.content}</pre></p>
				<p>날짜 : ${c.write_date}</p>
				<!-- 실제는 로그인 성공 후 관리자이거나 글쓴 본인인 경우만 삭제 버튼이 보임 -->
				<!-- 컨트롤러 가서 DB 삭제 후 다시 이곳으로 돌아온다.(나중에는 ajax로 실행한다.) -->
